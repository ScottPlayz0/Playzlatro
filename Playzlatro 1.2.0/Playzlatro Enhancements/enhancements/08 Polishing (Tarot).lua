SMODS.Consumable {
    ...
    loc_txt = {
        name = 'Polishing',
        text = { "Enhances {C:attention}1{} selected", "card into a", "{C:attention}Quartz Card{}" }
    },
    ...
    can_use = function(self, card) return G.hand and G.hand.highlighted and #G.hand.highlighted == 1 end,
    use = function(self, card, area, copier)
        ...
        G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.2, func = function()
            highlighted:flip(); play_sound('card1', 1); return true 
        end }))
        G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.4, func = function()
            ...
            if target_enhancement then highlighted:set_ability(target_enhancement) end
            return true 
        end }))
        G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.2, func = function()
            highlighted:flip(); play_sound('tarot2', 1, 0.6); return true 
        end }))
    end
}